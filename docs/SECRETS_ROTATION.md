...
| `NEXT_PUBLIC_POSTHOG_KEY`                | Vercel env (All envs)             | PostHog → Project Settings → Project API key. Rotating this resets analytics AND exception tracking — after rotate, confirm `/api/health/` is 200 and fire a test error. | Yes                                                             |
...
pushed to a public repo, or appearing in PostHog exception events):
