...
- [ ] `docs/SECURITY_AUDIT_2026-05-12.md` has all FIX items marked FIXED (includes the post-gym-tenant integration audit section at the bottom)
- [ ] `node scripts/rls-leak-test.js` exits 0 — **16 PASS / 0 FAIL / 4 WARN** baseline post-Week 3 Mission 1 (was 11/0/4 pre-integration; the 5 new phases cover gym SQL functions + gym/coach RLS). The 4 WARN entries are the documented payout/PII columns deferred in `LATER.md`.
...
- [ ] `/api/tribe-os/gym` PATCH owner-only check is in place (returns 403 'owner_only' for non-owner coaches)
- [ ] No deferred FIX items from the post-gym-tenant integration audit (Findings J–N) are blocking — finding M (`listCoachesForGym` returns only caller's row) is acceptable to ship since single-coach gyms are the beta norm; the multi-coach roster fix is post-merge work

### Migrations

- [ ] Every migration `056` through latest (currently **072**) is applied to the production Supabase project
...
- [ ] Migration 069 backfill verification queries return zero `unbackfilled_*` rows in production
- [ ] Migration 071 functions (`gym_revenue_totals`, `gym_revenue_buckets`) visible in `pg_proc` in production
- [ ] Migration 072 trigger (`client_attendance_sync_last_seen`) and indexes (`idx_clients_gym_last_seen`, `idx_clients_instructor_last_seen`) visible in production
...
### Gym-tenant integration (Weeks 1–3 of integration)

- [ ] `docs/WEEK_1_MISSIONS.md` all six rows show `✅ done`
- [ ] `docs/WEEK_2_MISSIONS.md` all six rows show `✅ done` and the leak-test gate checkbox is checked
- [ ] `getGymForUser(your_uid)` returns a non-null gym row in production (you should have one from migration 069's backfill)
- [ ] `/os/coaches` loads and shows you as the owner
- [ ] `/os/gym` loads and the form's editable fields submit successfully when you change the gym name to a real display value
- [ ] At least one new client created via `/os/clients/new` has `gym_id` populated (verifies the route handler refactor in Week 2 Mission 1 is actually populating both columns)
...
- [ ] PR description summarizes what shipped from `feature/tribe-os` (Weeks 1, 2, 3, 4 of Phase 2 + the gym-tenant integration on top)
