...
export async function insertPostComment(
  ...
  try {
    ...
    // comments_count is maintained by the trg_post_comments_count trigger
    // (migration 100). The previous three competing writers — an embedded
    // rpc() as a column value, an undeployed increment_post_comments RPC, and
    // a read-modify-write fallback — caused permanent drift and are removed.
...
export async function deletePostComment(supabase: SupabaseClient, commentId: string): Promise<DalResult<null>> {
  try {
    // Delete the comment. comments_count is recomputed by the
    // trg_post_comments_count trigger (migration 100) — no manual decrement,
    // which removes the old read-modify-write lost-update race.
...
export async function getPostCommentCount(supabase: SupabaseClient, postId: string): Promise<DalResult<number>> {
  try {
    const { data, error } = await supabase.from('instructor_posts').select('comments_count').eq('id', postId).single();
