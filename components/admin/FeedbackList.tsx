  ...
  return (
    <div className="space-y-3">
      {pendingFeedback.length > 0 && (
        <>
          ...
          {pendingFeedback.map((item) => (
            <div key={item.id} className="bg-white dark:bg-tribe-surface rounded shadow p-3">
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1">
                  <p className="text-sm font-bold text-tribe-dark">{item.title}</p>
                ...
                <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 text-xs rounded">
                  {item.type === 'feature_request' ? (language === 'es' ? 'Función' : 'Feature') : 'General'}
                </span>
              </div>
      ...
      {resolvedFeedback.slice(0, 5).map((item) => (
        <div key={item.id} className="bg-stone-50 dark:bg-tribe-dark rounded p-3 opacity-60">
