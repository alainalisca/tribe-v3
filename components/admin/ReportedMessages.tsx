  ...
  return (
    <div className="space-y-3">
      {pendingReports.length > 0 && (
        <>
          ...
          {pendingReports.map((report) => (
            <div key={report.id} className="bg-white dark:bg-tribe-surface rounded shadow p-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-1.5 border border-stone-300 dark:border-tribe-mid text-stone-700 dark:text-gray-300 text-xs rounded hover:bg-stone-50 dark:hover:bg-tribe-surface"
                >
      ...
      {resolvedReports.length > 0 && (
        <>
          <h3 className="text-sm font-bold text-tribe-dark mt-4">
            {language === 'es' ? 'Resuelto' : 'Resolved'} ({resolvedReports.length})
          </h3>
          {resolvedReports.slice(0, 5).map((report) => (
            <div key={report.id} className="bg-stone-50 dark:bg-tribe-dark rounded p-3 opacity-60">
