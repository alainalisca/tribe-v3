...
/**
 * Client-side interactivity for /communities.
 *
 * The parent Server Component fetches both the user's joined
 * communities and the full discover list in parallel, so initial
 * render has real data. This component owns:
 *   - Tab switching (communities / news / bulletin)
 *   - Search + sport filter (client-side over the server payload)
 *   - Sport pill "More/Less" toggle
 *
 * There is no client-side refetch — data is authoritative from the
 * server render. If a user action needs a fresh list (e.g. after
 * joining a community), wire a router.refresh() rather than
 * re-fetching here, so the server payload stays the single source
 * of truth.
 */
...
interface CommunitiesPageClientProps {
  initialUserCommunities: CommunityWithCreator[];
  initialAllCommunities: CommunityWithCreator[];
}

export default function CommunitiesPageClient({
  initialUserCommunities,
  initialAllCommunities,
}: CommunitiesPageClientProps) {
  const { language } = useLanguage();
  const t = getTranslations(language);

  const [activeTab, setActiveTab] = useState<'communities' | 'news' | 'bulletin'>('communities');
  const [userCommunities] = useState<CommunityWithCreator[]>(initialUserCommunities);
  const [allCommunities] = useState<CommunityWithCreator[]>(initialAllCommunities);
  const [filteredCommunities, setFilteredCommunities] = useState<CommunityWithCreator[]>(initialAllCommunities);
