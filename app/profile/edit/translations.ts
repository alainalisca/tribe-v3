export function getEditProfileTranslations(language: 'en' | 'es') {
  return {
    ...
    maxPhotos: language === 'es' ? 'Máximo 8 fotos permitidas' : 'Maximum 8 photos allowed',
    primaryPhoto: language === 'es' ? 'Foto de perfil principal' : 'Primary profile photo',
    changePhoto: language === 'es' ? 'Cambiar foto de perfil' : 'Change profile photo',
