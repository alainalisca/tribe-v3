  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-tribe-dark pb-32 safe-area-top">
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-6">
        <Link
          href="/settings"
          className="inline-flex items-center gap-1 text-stone-600 dark:text-gray-400 mb-4 text-sm hover:text-stone-900 dark:hover:text-white transition"
        >
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-tribe-dark dark:text-white mb-1">{t('adminPanel')}</h1>
        <p className="text-sm text-stone-600 dark:text-gray-400 mb-6 truncate">{user?.email}</p>

        <div
          className="flex gap-1 mb-6 border-b border-stone-300 dark:border-tribe-mid overflow-x-auto"
        ...
        >
          {tabs.map((tab) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 text-sm font-medium whitespace-nowrap relative transition ${
                activeTab === tab.id
                  ? 'border-b-2 border-tribe-green text-tribe-dark dark:text-white'
