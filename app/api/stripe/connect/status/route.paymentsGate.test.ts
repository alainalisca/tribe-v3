/**
 * PAY-01: the Stripe Connect status probe is behind the instructor payments
 * kill switch. StripeConnectBanner and the payout-settings page poll it; both
 * treat a non-ok response as "hide", so a 503 keeps every Connect entry point
 * dark while INSTRUCTOR_PAYMENTS_ENABLED is not exactly 'true'.
 */
...
async function get() {
  ...
  return GET(new NextRequest('https://tribe-v3.vercel.app/api/stripe/connect/status', { method: 'GET' }));
}

describe('connect status gate: flag unset or not "true"', () => {
...
describe('connect status gate: flag exactly "true"', () => {
