...
const TEMP_FILE = path.join(OUTPUT_DIR, 'temp-capture.png');
...
async function main() {
  console.log('Capturing session detail screenshot...\n');
  ...
  // Navigate to home and find Muay Thai session (it had messages)
  console.log('Finding session with messages...');
  ...
  for (const link of sessionLinks) {
    ...
    // Skip if it's already a chat link
    if (!href.includes('/chat')) {
      const sessionId = href.split('/session/')[1].split('/')[0];
      chatUrl = `https://tribe-v3.vercel.app/session/${sessionId}/chat`;
      break;
    }
  }

  // Try the Muay Thai session we saw earlier that had messages
  const muayThaiChatUrl = 'https://tribe-v3.vercel.app/session/c7e4d0a8-9eac-4aa9-94a0-f0693b1aaecf/chat';
  console.log(`Trying Muay Thai chat: ${muayThaiChatUrl}`);
  await page.goto(muayThaiChatUrl, { waitUntil: 'networkidle2', timeout: 30000 });
  await new Promise(resolve => setTimeout(resolve, 3000));
  await dismissModals(page);

  // Check if this chat has messages
  const noMessages = await page.$('text/No messages yet');
  if (noMessages) {
    console.log('No messages in this chat, trying session detail instead...');
    // Go to session detail page instead
    await page.goto('https://tribe-v3.vercel.app/session/c7e4d0a8-9eac-4aa9-94a0-f0693b1aaecf', { waitUntil: 'networkidle2', timeout: 30000 });
    await new Promise(resolve => setTimeout(resolve, 3000));
    await dismissModals(page);
  }
